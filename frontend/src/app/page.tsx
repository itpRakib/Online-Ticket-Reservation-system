      ...
      <section className="mx-auto max-w-7xl px-4 pt-12 pb-8 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          >
            {user ? (
              <>
                <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} className="inline-flex items-center space-x-2 rounded-full border border-[#6F9526]/30 bg-[#6F9526]/10 px-4 py-1.5 text-xs text-[#2A5B60] font-bold shadow-sm">
                  <Sparkles className="h-3.5 w-3.5 text-[#6F9526]" />
                  <span>{t("Assalam-o-Alaikum, ", "আসসালামু আলাইকুম, ") + (user.first_name || user.username) + "!"}</span>
                </motion.div>
                
                <motion.h1 variants={{ hidden: { opacity: 0, y: 20, filter: 'blur(6px)' }, visible: { opacity: 1, y: 0, filter: 'blur(0px)' } }} className="text-4xl font-extrabold tracking-tight sm:text-5xl text-[#2A5B60] leading-tight" style={{ fontFamily: 'var(--font-heading), sans-serif' }}>
                  {t("PLAN YOUR NEXT ", "আপনার পরবর্তী ")} <br className="hidden sm:inline" />
                  <span className="bg-gradient-to-r from-[#2A5B60] via-[#6F9526] to-[#C5D050] bg-clip-text text-transparent">
                    {t("TRANSIT ROUTE", "ট্রানজিট রুট পরিকল্পনা")}
                  </span>
                </motion.h1>
                
                <motion.p variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} className="text-[#444E29] text-sm sm:text-base leading-relaxed max-w-xl mx-auto lg:mx-0 font-medium">
            ...
            <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} className="flex flex-wrap gap-4 items-center justify-center lg:justify-start text-xs font-semibold text-[#444E29]">
              <span className="flex items-center space-x-1.5"><CheckCircle2 className="h-4 w-4 text-[#6F9526]" /> <span>{t("NID Verified", "এনআইডি যাচাইকৃত")}</span></span>
              <span className="flex items-center space-x-1.5"><CheckCircle2 className="h-4 w-4 text-[#6F9526]" /> <span>{t("SIM SMS OTP", "সিম ওটিপি")}</span></span>
              <span className="flex items-center space-x-1.5"><CheckCircle2 className="h-4 w-4 text-[#6F9526]" /> <span>{t("bKash/Nagad checkout", "বিকাশ/নগদ পেমেন্ট")}</span></span>
            </motion.div>
          </motion.div>
        ...
        <GlowCard glowColor="emerald" intensity="low">
          <div className="glass-panel rounded-3xl p-6 sm:p-8 shadow-2xl relative border border-[var(--border)] bg-[var(--bg-raised)]/40 backdrop-blur-md">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              type="submit"
              className="w-full rounded-xl p-4 font-extrabold text-[#2A5B60] hover:text-white hover:bg-[#6F9526] flex items-center justify-center space-x-2 cursor-pointer shadow-md transition-all duration-300 border border-[#C5D050]"
              ...
              {(['balanced', 'budget', 'comfort', 'speed'] as const).map(mode => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-extrabold uppercase tracking-wider transition-all duration-300 border cursor-pointer ${
                    simMode === mode
                      ? 'bg-[#C5D050] text-[#2A5B60] border-[#C5D050] shadow-md scale-105'
                      : 'bg-[#F3F3F3] dark:bg-[#1C2E31] text-[#444E29] dark:text-slate-300 border-[#2A5B60]/20 hover:border-[#6F9526] hover:text-[#2A5B60] hover:bg-[#6F9526]/10 shadow-sm'
                  }`}
                >
                  {mode}
                </button>
