      ...
      <GlowCard glowColor="emerald" intensity="medium">
        ...
        >
          <div className="text-center">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl text-white shadow-md bg-gradient-to-tr from-cyan-500 to-indigo-600">
              <Ticket className="h-6 w-6 rotate-12" />
            </div>
            <h2 className="mt-6 text-3xl font-black text-slate-900 dark:text-white tracking-tight" style={{ fontFamily: 'var(--font-heading), sans-serif' }}>
            ...
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300 font-medium">
              {otpSent ? (
                ...
                <>
                  ...
                  Or{' '}
                  <Link href="/auth/register" className="font-bold text-cyan-500 hover:text-indigo-500 transition-colors underline">
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            {!otpSent ? (
              <div className="space-y-4 rounded-md shadow-sm">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 dark:text-cyan-400 uppercase tracking-wider">Username / Email / Phone</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="block w-full pl-10 pr-3.5 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:border-cyan-500 focus:outline-none transition-colors shadow-xs font-medium"
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 dark:text-cyan-400 uppercase tracking-wider">Password</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-slate-400">
                    ...
                    <input
                      ...
                      className="block w-full pl-10 pr-10 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:border-cyan-500 focus:outline-none transition-colors shadow-xs font-medium"
                  ...
                  <div className="flex justify-end mt-1.5">
                    <Link
                      ...
                      className="text-xs font-bold text-cyan-500 hover:text-indigo-500 transition-colors"
                    >
                      Forgot Password?
                    </Link>
                  </div>
              ...
              <div className="space-y-4">
                <div className="text-sm text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 leading-relaxed font-medium">
                  We have dispatched a 6-digit security code to your verified Gmail: <span className="font-extrabold text-cyan-500 font-mono">{emailAddress}</span>.
                </div>
                ...
                {otpActual && (
                  <motion.div
                    ...
                    className="p-4 rounded-xl bg-cyan-500/10 border border-cyan-500/30 space-y-2 shadow-md"
                  >
                    <div className="flex items-center space-x-2 text-cyan-500 text-xs font-bold uppercase tracking-wider">
                      <Mail className="h-4 w-4 text-cyan-500 animate-bounce" />
                      <span>Gmail Server Delivery</span>
                    </div>
                    <div className="text-xl font-black text-cyan-400 font-mono tracking-widest text-center mt-1">
                      {otpActual}
                    </div>
                    <button
                      type="button"
                      ...
                      className="w-full mt-2 py-2 rounded-lg text-white text-xs font-bold transition-all cursor-pointer shadow-sm hover:opacity-90 bg-gradient-to-r from-cyan-500 to-indigo-600"
                    >
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 dark:text-cyan-400 uppercase tracking-wider">Gmail Verification Code</label>
                  <input
                    type="text"
                    maxLength={6}
                    required
                    ...
                    className="block w-full py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 tracking-widest text-center font-bold placeholder-slate-400 focus:border-cyan-500 focus:outline-none transition-colors shadow-xs"
                    placeholder="XXXXXX"
                  />
                  <div className="flex justify-between items-center text-xs mt-2 text-slate-500 dark:text-slate-400">
                    <span>Expires in: <span className={`font-bold ${timer < 30 ? 'text-red-500' : 'text-cyan-500'}`}>{Math.floor(timer / 60)}:{(timer % 60).toString().padStart(2, '0')}</span></span>
                    <button
                      type="button"
                      disabled={loading || timer > 90}
                      onClick={handleResendOtp}
                      className="text-cyan-500 hover:text-indigo-500 font-bold disabled:text-slate-400 disabled:cursor-not-allowed"
                    >
            ...
            <div className="space-y-4">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full rounded-xl py-3.5 font-black text-white bg-gradient-to-r from-cyan-500 via-indigo-600 to-emerald-500 hover:opacity-95 flex items-center justify-center space-x-2 transition-all shadow-md cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed border border-cyan-400/30"
              >
                {loading ? (
                  <>
              ...
              {otpSent && (
                <div className="text-center mt-2">
                  <button
                    ...
                    className="text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-cyan-500 transition-colors cursor-pointer"
                  >
